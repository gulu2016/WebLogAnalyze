...
/*
* 1.map函数的输入输出是怎样的
* 2.reduce函数的大致过程(2.1-2.3)
* 3.如何从访问记录中分辨切割出单次访问(3.1-3.6)
* 4.reduce函数的输入输出过程是怎样的
* */
public class ClickStream {


    //1.map函数的输入是一行WebLogBean.toString()的记录
    //输出是<WebLogBean.ip,WebLogBean>
	...
	//4.reduceTask的输入：<WebLogBean.ip,WebLogBean>
    //reduceTask的输出：<null,(session,ip,WebLogBean的属性,当前访问的步数stepNum)>
	static class ClickStreamReducer extends Reducer<Text, WebLogBean, NullWritable, Text> {
		...
		protected void reduce(Text key, Iterable<WebLogBean> values, Context context) throws IOException, InterruptedException {
			...
			try {
			    //2.1将每个传入的WebLogBean对象的副本加入ArrayList中
				...
				for (int i = 0; i < beans.size(); i++) {
					...
					if (1 == beans.size()) {
						...
						v.set(session+"\001"+key.toString()+"\001"+bean.getRemote_user() + "\001" + bean.getTime_local() +
								"\001" + bean.getRequest() + "\001" + step + "\001" + (60) + "\001" + bean.getHttp_referer() + "\001" + bean.getHttp_user_agent() + "\001" + bean.getBody_bytes_sent() + "\001"
								+ bean.getStatus());
					...
					// 3.2如果不止1条数据，则将第一条跳过不输出，遍历第二条时再输出
					if (i == 0) {
						continue;
					}
					...
					// 3.4如果本次-上次时间差<30分钟，则输出前一次的页面访问信息
                    ...
                    // 3.5如果本次-上次时间差>30分钟，则输出前一次的页面访问信息且将step重置，以分隔为新的visit
					else {
